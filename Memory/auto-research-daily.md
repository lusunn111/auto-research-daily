...
- GitHub Actions 已在线运行：使用 `Asia/Shanghai` 原生时区在北京时间 12:30 主触发，15:17 和 18:43 进行错峰恢复检查；只有当天日报快照与邮件通知状态都已提交时才跳过，否则自动补跑。生成路径白名单提交，同次 GitHub Pages 部署成功后才通过 QQ 邮箱发送 Gmail 通知，并单独记录通知状态。
...
- 网页信息架构已拆为两个时间轴：`daily/YYYY-MM-DD/` 按系统运行日期保存日报，`archive/YYYY-MM/` 按论文首次发表月份归档。首页只呈现最新日报，长期去重库位于 `library/`，稳定主题、固定标签、搜索和单篇永久详情均为独立页面。
- `web/public/` 在审阅的 `vla-wam-daily` 上游版本中只承载论文图片等静态资产，页面结构由累计归档数据生成；本项目对应地继续把图片放在 `site/figures/`，不把静态资源目录误当作归档数据源。
- 固定标签采用项目内别名注册表归一化，当前覆盖动作预测、具身数据、高效推理、评测、通用机器人、模型量化、导航、物理智能、策略学习、机器人学习、机器人操作、仿真、推测解码、视觉语言和世界建模。模型自由标签仍保留为解读元数据，但不直接生成网址。

## 已验证

- Python 3.13 环境中 Ruff 静态检查通过。
- 35 项离线测试通过，覆盖 Figure 1/2 结构解析、嵌套 Figure 归属、跨论文图片拒绝、编码路径穿越、图片真实解码、缓存失败降级、全局下载预算、原子缓存、日报快照、长期库、稳定主题、固定标签、月份归档、搜索、永久详情、画廊默认折叠结构，以及缓存命中场景下按完整可发布批次计算分析失败率。
- 桌面与手机视口的浏览器验证通过：当前首页 26 张卡片默认折叠，长期库汇总 43 篇当前版本论文，8 个月份归档、标签筛选、单篇展开和永久详情均正常；手机端无横向溢出，未发现脚本或页面覆盖层错误。
...
- 每日任务以北京时间 12:30 为主运行时间；如果 GitHub 未投递或流水线未完整交付，15:17 与 18:43 会根据日报快照和邮件状态补偿。网页在分析、发布和部署完成后更新，邮件在网页部署成功后发送。
