...
首页采用论文画廊，而不是连续铺开 30 至 40 篇长报告。每张卡片默认只展示论文原图或主题占位图、题目、作者、核心洞察、推荐理由和标签；点击“展开完整解读”后，当前论文会横跨整行并显示全部结构化研究笔记、Figure 1 / Figure 2 和证据。页面也提供筛选结果批量展开与全部收起。

网页在模型生成的细粒度 `primary_topic` 之外增加一层稳定展示分类：视觉语言动作（VLA）、世界动作模型（WAM）、世界模型（World Model）、机器人学习与控制、数据集（Dataset）、基准评测（Benchmark）、高效推理、模型量化（Quantization）和前沿探索。该分类由题目、摘要、标签和命中术语确定性映射，不修改原始解读，也不增加大模型调用。当天没有论文的分类不会显示。

分类、关键词、编辑层级、证据范围、最低相关性和论文配图可以组合筛选，筛选状态会写入网址查询参数，刷新或分享后仍能复现。点击论文标签会把该标签送入关键词筛选。无 JavaScript 时，页面仍显示全部论文和可展开的研究笔记。
...
持续集成完全使用固定夹具，不访问 arXiv、Zotero 或真实大模型。每日工作流以 `Asia/Shanghai` 原生时区在 12:30 主触发，并在 15:17、18:43 进行错峰恢复检查。只有当天日报快照与邮件通知状态都已提交时，恢复检查才直接退出；缺少任一结果时自动补跑。逐篇论文统一使用低成本的 V4 Flash 非思考模式；未来的跨论文综合最多使用 V4 Flash 低强度思考，不启用 V4 Pro。执行测试后仅提交允许的生成目录，并在同一次运行中部署 GitHub Pages。运行报告会记录真实输入和输出令牌数。
